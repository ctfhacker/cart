...
| AT-M3-01 | M3 | CLI encode/decode parity | `random_10MB.bin`, `zeros_128MiB.bin` | `scripts/seed_cart_artifacts.py` then `cargo test --package cart-core -- test decode_seeded_artifact_matches_original` | Rust decode output matches seeded source bytes; exit success | Round-trip diff ≤0 bytes; command completes ≤60 s once CLI wiring lands.
